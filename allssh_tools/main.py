...
def run_allssh_command():
    ...
    allssh_command = input("\nEnter the command to run on all clusters (e.g. 'ip addr'): ").strip()
    if not allssh_command:
        print("No command entered. Cancelling.")
        return

    cluster_file = "allssh_tools/shared_cohesity_clusters.txt"
    try:
    ...
    # Fixed: Use escaped quotes or single quotes for bash -c
    commands = [f"sudo -u cohesity bash -c '/home/cohesity/bin/allssh.sh \"{allssh_command}\"'"]
